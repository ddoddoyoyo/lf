...
(function(global, factory) {
	...
	$.fn.fullpage = function(options) {
		...
		options = $.extend({
			...
			//design
			controlArrows: false,//defalt: true
		...
		*/
		function styleSection(section, index){
			...
			//section.css('height', windowsHeight + '%');
		...
		*/
		...
		function scrollHandler(){
			...
			if(!options.autoScrolling || options.scrollBar){
				...
				var screen_mid = currentScroll + (windowsHeight / 2.0);
		...
		*/
		function isCompletelyInViewPort(movement){
			...
			if(movement == 'up'){
				...
				return bottom >= ($window.scrollTop() + windowsHeight);
			}
		...
		function touchMoveHandler(event){
			...
			if (!checkParentForNormalScrollElement(event.target) && isReallyTouch(e) ) {
				...
				//vertical scrolling (only when autoScrolling is enabled)
					...
					if (Math.abs(touchStartY - touchEndY) > (windowsHeight / 100 * options.touchSensitivity)) {
		...
		*/
		function performHorizontalMove(slides, v, fireCallback){
			...
			//slideWidth = $('#fullpage .slidePage#'+ _slide +' .slideImg img').width();
		...
		function resizeHandler(){
			...
			if (isTouchDevice) {
				...
				if (!activeElement.is('textarea') && !activeElement.is('input') && !activeElement.is('select')) {
					...
					var currentHeight = windowsHeight;
