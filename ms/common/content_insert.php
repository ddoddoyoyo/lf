<?php
	...
	if( $_FILES["CON_IMAGE"][size] > 0 ) {
		// 업로드 파일 제한 확장자 추가 가능
		$EXT_CHECK = array("php", "php3", "htm", "html", "cgi", "perl");	
		if( $EXT_TMP = explode( ".", $_FILES["CON_IMAGE"][name])) {	 
		...
		$CON_IMAGE	= "LF_MS_CON_".date("YmdHis").time().".".$EXT_TMP[1];	//변환파일명

		if( !@move_uploaded_file($_FILES["CON_IMAGE"][tmp_name], $IMG_DIR."hyundai/lf/".$CON_IMAGE) ) { 
			//$tools->errMsg("파일 업로드 에러"); 
		}else { 
			@unlink($_FILES["CON_IMAGE"][tmp_name]);
			chmod($IMG_DIR."hyundai/lf/".$CON_IMAGE,0755);
		}	
	}else{
		$CON_IMAGE = "";
	}
	...
	try 
	{
		...
		$sql = "INSERT INTO LMS_CONTENTS
			...
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':LMS_SEQ',$LMS_SEQ);
			$stmt->bindParam(':LMS_CON_TEXT',$CON_TEXT);
			$stmt->bindParam(':LMS_CON_TITLE_IMG',$CON_IMAGE);
